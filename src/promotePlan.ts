...
export async function generatePromotePlan(opts: { root: string; minRefs: number }): Promise<PromotePlan> {
  ...
  const candidates = [...refs.entries()].filter(([, c]) => c >= opts.minRefs).sort((a, b) => b[1] - a[1]);
...
export async function applyPromotePlan(opts: { root: string; planFile: string; write: boolean }): Promise<number> {
  ...
  for (const move of plan.moves) {
    ...
    if (!fromAbs) {
      problems.push(`Missing stub file for '${move.from}' (searched ${fromCandidates.join(', ')})`);
    }

    const toAbs = path.join(config.vaultRoot, move.toRelPath);
    if (!move.toRelPath.endsWith('.md')) {
      problems.push(`Destination must end with .md: ${move.toRelPath}`);
    }
    if (fs.existsSync(toAbs)) {
      problems.push(`Destination already exists: ${move.toRelPath}`);
    }
  }
  ...
  if (problems.length) {
    ...
    if (problems.length > 30) {
      console.error(pc.dim(`…and ${problems.length - 30} more`));
    }
    return 1;
  }
  ...
  for (const m of preview) {
    console.log(`- ${m.from} -> ${m.toRelPath}`);
  }
  if (plan.moves.length > preview.length) {
    console.log(pc.dim(`…and ${plan.moves.length - preview.length} more`));
  }
  ...
  for (const move of plan.moves) {
    ...
    const fromAbs = fromCandidates.find((p) => fs.existsSync(p));
    if (!fromAbs) {
      throw new Error(`Invariant: missing stub '${move.from}'`);
    }
